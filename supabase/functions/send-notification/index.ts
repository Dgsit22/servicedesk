...
// Save as: supabase/functions/send-notification/index.ts in your GitHub repo
...
async function sendEmail(to: string, subject: string, html: string) {
  ...
  const smtpRes = await fetch("https://smtp.deno.dev/send", {
    ...
    body: JSON.stringify({ host: SMTP_HOST, port: SMTP_PORT, username: SMTP_USER, password: SMTP_PASS, from: FROM_EMAIL, to, subject, html }),
  });
...
function priorityColor(p: string) {
  const map: Record<string, string> = { critical: "#a61e1e", high: "#8a4500", medium: "#1e40af", low: "#14532d" };
  return map[p] || "#333";
}

function statusLabel(s: string) {
  const map: Record<string, string> = { inprogress: "In Progress", pending_approval: "Pending Approval", waiting_for_info: "Waiting for Info", resolved: "Resolved", closed: "Closed", open: "Open" };
...
serve(async (req) => {
  ...
  try {
    ...
    if (type === "test") {
      ...
      const name = payload.admin_name || "Admin";
      if (to) {
        await sendEmail(to, "✅ ServiceDesk — Email Test Successful",
          baseTemplate("Email Configuration Working!", `
            <p style="font-size:13px;color:#6b6860;margin-bottom:16px">Hi ${name},</p>
            <p style="font-size:13px;color:#6b6860;margin-bottom:16px">Your ServiceDesk email notifications are working correctly.</p>
            <div style="background:#f0fdf4;border-radius:10px;padding:16px;border-left:4px solid #16a34a;margin-bottom:16px">
              <div style="font-size:14px;font-weight:700;color:#14532d">✅ Configuration verified</div>
              <div style="font-size:13px;color:#166534;margin-top:4px">All notification types are ready to send.</div>
            </div>
            <p style="font-size:12px;color:#a09e9a">Tested from Admin → Notification Center</p>
          `)
        );
        emailsSent++;
      }
    ...
    if (type === "ticket_raised") {
      ...
      if (requester_email) {
        await sendEmail(requester_email,
          ...
          baseTemplate(`✅ Request Received — ${ticket_no}`, `
            ...
            <p style="font-size:13px;color:#6b6860;margin-bottom:16px">Your IT request has been submitted. Our team will review it shortly.</p>
    ...
    if (type === "assigned") {
      ...
      if (technician_email) {
        await sendEmail(technician_email,
    ...
    if (type === "status_changed") {
      ...
      if (requester_email && new_status) {
        ...
        if (new_status === "inprogress") {
          ...
          message = "IT needs more information from you. Please open the ticket and add a comment.";
